...
export function AdminSidebar({ userEmail }: AdminSidebarProps) {
    ...
    return (
        <>
            {/* Mobile / Tablet Static Bar (Shows when menu is closed) */}
            <div className="lg:hidden fixed top-0 left-0 w-full h-[70px] bg-[#111111] text-white z-40 flex items-center px-6 justify-between border-b border-white/10 shadow-lg">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => setIsMenuOpen(!isMenuOpen)}
                        className="text-white hover:text-[#E4192B] transition-colors"
                    ...
                    <span className="text-2xl font-bold tracking-tight" style={{ fontFamily: '"Adobe Garamond Pro", "EB Garamond", serif' }}>
                        Adverbe<span className="text-[#E4192B]">.</span>
                    </span>
                </div>
                <Link href="/" target="_blank" className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors">
                    <Globe className="w-3.5 h-3.5 text-white/70" />
                </Link>
            </div>

            {/* Desktop Sidebar (Fixed) */}
            <aside className="hidden lg:flex w-[260px] bg-[#111111] text-white flex-col fixed h-full z-10" suppressHydrationWarning>
                {/* Brand Desktop */}
            ...
            {/* Mobile / Tablet Drawer (Independent Overlay) */}
            <AnimatePresence>
                {isMenuOpen && (
                    <>
                        {/* Backdrop overlay */}
                        ...
                        <motion.aside
                            ...
                            className="fixed top-0 left-0 h-full w-[260px] bg-[#111111] text-white z-50 flex flex-col pt-5 shadow-2xl lg:hidden"
                            suppressHydrationWarning
                        >
                            <div className="px-7 pt-2 pb-6 border-b border-white/10 mb-2 mt-2">
                                <span className="text-2xl font-bold tracking-tight" style={{ fontFamily: '"Adobe Garamond Pro", "EB Garamond", serif' }}>
                                    Adverbe<span className="text-[#E4192B]">.</span>
                                </span>
                                <p className="text-[11px] text-white/30 mt-1 uppercase tracking-[0.15em] font-medium">Admin Console</p>
                            ...
                            <div className="px-4 py-5 border-t border-white/[0.06] mt-4">
                                <div className="flex items-center gap-3 px-3 mb-4">
                                    <div className="w-8 h-8 rounded-full bg-[#E4192B] flex items-center justify-center text-white text-xs font-bold uppercase">
                                        {userEmail?.charAt(0) || 'A'}
                                    </div>
